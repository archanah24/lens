...
public class ITCostTests extends BaseTestClass {
  ...
  public static final String COST_3 = QueryInventory.getQueryFromInventory("HIVE.COST_5");
  public static final String COST_2 = QueryInventory.getQueryFromInventory("HIVE.COST_5");
  ...
  @Test(enabled = true, groups= "user-cost-ceiling")
  public void diffUserConcurrentPriorityThrottling() throws Exception {
    ...
    try {
      ...
      for (int i = 1; i <= 3; i++) {
        ...
        handleList.add((QueryHandle) qHelper.executeQuery(COST_5, "", session1).getData());
      ...
      while (counter < 500) {
        List<QueryHandle> running = qHelper.getQueryHandleList(null, "RUNNING", "all", sessionHandleString, null, null,
            hiveDriver);
        List<QueryHandle> queued = qHelper.getQueryHandleList(null, "QUEUED", "all", sessionHandleString, null, null,
            hiveDriver);
        ...
        if (running.size() == 0 && queued.size() == 0) {
          break;
        }
        Thread.sleep(5000);
      }
      ...
      List<QueryHandle> queued = qHelper.getQueryHandleList(null, "QUEUED", "all", sessionHandleString, null, null,
          hiveDriver);
      Assert.assertEquals(0, queued.size());
  ...
  */
  ...
  public void queueNumberChangeWithInSamePriority() throws Exception {
    ...
    String longRunningQuery = String.format(QueryInventory.getQueryFromInventory("HIVE.SLEEP_COST_95"), "10");
    ...
    try {
      ...
      Assert.assertEquals(lq1.getStatus().getStatus(), QueryStatus.Status.RUNNING);
      ...
      Assert.assertEquals(lq1.getStatus().getStatus(), QueryStatus.Status.SUCCESSFUL);
      Assert.assertEquals(lq2.getStatus().getStatus(), QueryStatus.Status.RUNNING);
